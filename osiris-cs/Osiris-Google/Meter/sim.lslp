string scriptName = "sim";
string secureKey="7yxpZa2Rfq/wG/LRGidWJCy8BAw=";
string myKey="qOcN+LnvG64jB3swOa8kdmB8XL4=";
...
// A - armor ( makes players harder to hit, with a corresponding drop in stamina) â€“ stamina drop will be a function of how good the protection is, so the heaviest armor will drop stamina the most. This would be a wearable attachment, and it is recommended that it be visible.
state armor {
    state_entry() {
    }
}
// G - area damage (bombs, grenades, etc which original from the player) â€“ 25 points of area damage, 10 meter radius around the avatar, meter enforced 30 second delay between hits.
state areadmg {
    state_entry() {
    }
}
// R - API Reflect (combat) â€“ shielding that reflects melee/ranged damage back to the originator for 60 seconds.  30 minute charge between uses.
state reflect {
    state_entry() {
    }
}
